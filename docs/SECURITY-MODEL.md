...
| Deployment gating | Staging from `main`; production from published immutable GitHub Releases with GitHub-verified signed annotated `v*` tags | Environment branch/tag policy, immutable releases, signed-tag workflow verification, and workflow logic | Blocks deploy | Without Enterprise environment reviewers or wait timers, the hard gate is release publication plus tag policy, not human environment approval. |
